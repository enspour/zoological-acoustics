...
const prefix = (changes: KuduChange[], prefix: PropertyKey) => {
...
export interface KuduChangeEdit {
...
export interface KuduChangeAppend {
  kind: 'N' | 'AN'; // N - new property, AN - new array element
  path: PropertyKey[];
  value: any;
}

export interface KuduChangeRemove {
  kind: 'R' | 'AR'; // R - remove property, AR - remove array element
  path: PropertyKey[];
  value: any;
}

export type KuduChange = KuduChangeEdit | KuduChangeAppend | KuduChangeRemove;

export class KuduDiffer {
  static diff<L, R>(
    lhs: L,
    rhs: R,
  ...
  static diffByValue<L, R>(lhs: L, rhs: R): KuduChange[] {

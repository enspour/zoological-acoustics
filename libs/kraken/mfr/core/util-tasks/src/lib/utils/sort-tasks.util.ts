...
import { Task, TaskWithColumn } from '@kraken/domain';
...
const comparatorByStartDate = (a: Task, b: Task) =>
  new MkDate(a.startDate).valueOf() - new MkDate(b.startDate).valueOf();

const comparatorByEndDate = (a: Task, b: Task) =>
  new MkDate(a.endDate).valueOf() - new MkDate(b.endDate).valueOf();

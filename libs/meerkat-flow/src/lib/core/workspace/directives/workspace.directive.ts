...
import { resizeObservable } from '@meerkat-ng-web-apis';
...
import { MkFlowWorkspaceScale, MkFlowWorkspaceScroll } from '../interfaces';

@Directive()
export class MkFlowWorkspaceDirective {
  private document = inject(DOCUMENT);
  private elementRef = inject<ElementRef<HTMLElement>>(ElementRef);

  public scroll = model<MkFlowWorkspaceScroll>({ x: 0, y: 0 });
  public scale = model<MkFlowWorkspaceScale>(1);
  ...
  public setScroll(scroll: MkFlowWorkspaceScroll) {
    this.scroll.set(scroll);
  }

  public setScale(scale: MkFlowWorkspaceScale) {
